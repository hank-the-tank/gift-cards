...
def view_orders(db: Session = Depends(db.db_connection)):
...
def view_order(id: UUID, db: Session = Depends(db.db_connection)):
...
def delete_order(id: UUID, db: Session = Depends(db.db_connection)):
...
def update_order(
    id: UUID, request: schemas.OrderRequestType, db: Session = Depends(db.db_connection)
):
    order = db.query(m.Order).filter(m.Order.id == id)
    if not order.first():
        raise HTTPException(
